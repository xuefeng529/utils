#ifndef PLUGINS_REDIS_CLUSTERCLIENT_H
#define PLUGINS_REDIS_CLUSTERCLIENT_H
...
public:
	...
	Status(const redisClusterContext* ctx, const redisReply* reply);
...
private:
	const redisClusterContext* ctx_;
    const redisReply* reply_;
};

/// Note: ClusterClient not thread-safe
class ClusterClient : boost::noncopyable
{
public:
	ClusterClient();
	~ClusterClient();
    ...
    bool connect(const std::string& hosts, const std::string& password = std::string());
    bool ping();
...
#endif // PLUGINS_REDIS_CLUSTERCLIENT_H
